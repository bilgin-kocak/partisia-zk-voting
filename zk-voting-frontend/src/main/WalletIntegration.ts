...
export const updateContractState = () => {
  ...
  CLIENT.getContractData<RawZkContractData>(address).then((contract) => {
    if (contract != null) {
      ...
      administrator.innerHTML = `${state.owner.asString()}`;
      ...
      console.log("state.voteResult", state.voteResult);
      averageSalaryResult.innerHTML = `${state.voteResult ?? "None"}`;

      const votesAgaintsResult = <HTMLElement>document.querySelector("#votes-against");
      votesAgaintsResult.innerHTML = `Votes Against: ${state.voteResult?.votesAgainst ?? "None"}`;


      const votesForResult = <HTMLElement>document.querySelector("#votes-for");
      votesForResult.innerHTML = `Votes For: ${state.voteResult?.votesFor ?? "None"}`;

      const numEmployees = <HTMLElement>document.querySelector("#num-employees");
      let deadlineVotingTime = '';
      if (state.deadlineVotingTime.toString() ?? false){
        
        deadlineVotingTime = new Date(Number(state.deadlineVotingTime.toString())).toLocaleString()
        console.log('deadlineVotingTime', deadlineVotingTime)

      }
      numEmployees.innerHTML = `${deadlineVotingTime ?? "None"}`;
